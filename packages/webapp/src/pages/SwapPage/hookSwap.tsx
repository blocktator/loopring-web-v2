          ...
          if (response && response.hash && response?.status) {
            ...
            switch (resp.orderDetail.status) {
              ...
              case sdk.OrderStatus.cancelled:
                const baseAmount = sdk
                  .toBig(resp.orderDetail.volumes.baseAmount)
                  .toNumber();
                const quoteAmount = sdk
                  .toBig(resp.orderDetail.volumes.quoteAmount)
                  .toNumber();
                  ...
                  {response.code && (
                    <Tooltip title={response.message}>
                      <Link
                        color="textSecondary"
                        target="_blank"
                        href={`https://docs.loopring.io/en/dex_apis/applyApiKey.html?q=${response.code}%20`}
                      >
                        {response.code}
                      </Link>
                    </Tooltip>
